...
def test_schema_layer_does_not_import_qt() -> None:
    """`app.agent.schema` / `app.agent.validate` を import しても PySide6 が入らない。"""
    for module in ("app.agent.schema", "app.agent.validate"):
...
def test_box_key_on_endpoint_type_returns_a_corrected_call() -> None:
    ...
    assert error.extra["corrected_call"]["arguments"]["moves"][0]["id"] == 23
